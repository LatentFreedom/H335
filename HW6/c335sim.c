...
{
  if (landscape) {
    screen = SDL_SetVideoMode(ST7735_height * zoom, 
			      ST7735_width * zoom, 
			      24, 
			      SDL_SWSURFACE);
    ...
    screen = SDL_SetVideoMode(ST7735_width * zoom, 
			      ST7735_height * zoom, 
			      24, 
			      SDL_SWSURFACE);
...
int SDL_main(int argc, char *argv[])
{
  ...
  //Start SDL // check for correct input number
  if(argc != 3) {
    printf("Need a scape mode and file.\n");
    return result;
  }
  ...
  //Set up screen
  //check for correct scape inputs
  if(strcmp("PORTRAIT",argv[1]) == 0 || 
     strcmp("portrait",argv[1]) == 0) {
      
    printf("Portrait selected.\n");
    init_screen(portrait);
  } else if(strcmp("LANDSCAPE",argv[1]) == 0 || 
	    strcmp("landscape",argv[1]) == 0) {
    
    printf("Landscape selected.\n");
    init_screen(landscape);
  } else {
    printf("Invalid scape.\n");
    return 0;
  }
